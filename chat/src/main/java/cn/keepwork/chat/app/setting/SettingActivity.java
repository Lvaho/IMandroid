...
import cn.keepwork.chat.app.main.SplashActivity;
import cn.keepwork.chat.app.AppService;
import cn.keepwork.chat.app.DateUtil;
...
public class SettingActivity extends WfcBaseActivity {
        ...
        switch (requestCode) {
            case REQUEST_IGNORE_BATTERY_CODE:
                if (resultCode == RESULT_CANCELED) {
                    Toast.makeText(this, "允许之讯IM后台运行，更能保证消息的实时性", Toast.LENGTH_SHORT).show();
                }
                break;
            default:
    ...
    void exit() {
        ...
        AppService.Instance().changeName("离线"+time, new SimpleCallback<Void>() {
            @Override
            public void onUiSuccess(Void aVoid) {
                Toast.makeText(SettingActivity.this,"在线状态修改成功",Toast.LENGTH_LONG).show();
            }

            @Override
            public void onUiFailure(int code, String msg) {
                Toast.makeText(SettingActivity.this,"在线状态修改失败",Toast.LENGTH_LONG).show();
            }
        });
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                ...
                if (!pm.isIgnoringBatteryOptimizations(packageName)) {
                    ...
                    Toast.makeText(this, "已忽略电池优化，允许之讯IM后台运行，更能保证消息的实时性", Toast.LENGTH_SHORT).show();
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
